...
future<void> sock_connector::launch()
{
	...
	auto th_fn = [this]()
	{
		...
		if (srt_epoll_wait(m_epoll_conn, rd_ready, &rd_len, wr_ready, &wr_len, POLL_TIMEOUT_MS, 0, 0, 0, 0) >= 0)
		{
	...
	return std::async(std::launch::async, th_fn);
}
//
